...
export default function CameraCapture({
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      setErrorMsg(
        "Không thể mở trực tiếp webcam (Có thể do chế độ bảo mật trình duyệt hoặc thiếu quyền camera). Vui lòng chuyển sang chọn file chụp ảnh."
      );
        ...
        {photo ? (
          ...
          <div className="relative w-full aspect-[4/3] max-h-[220px] bg-slate-900 group">
            ...
            <div className="absolute inset-0 bg-black/45 opacity-0 group-hover:opacity-100 transition duration-150 flex items-center justify-center gap-2">
              ...
              <button
                type="button"
                onClick={startCamera}
                className="px-3 py-1.5 bg-[#10b981] text-white text-[10px] font-bold rounded-lg flex items-center gap-1 cursor-pointer shadow-md active:scale-95 transition"
              >
          ...
          <div className="p-4 text-center space-y-3.5 flex flex-col items-center">
            <div className="flex gap-2.5">
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-[10.5px] font-bold transition active:scale-95 cursor-pointer border shadow-sm ${
                  isDark ? 'bg-[#10b981]/15 text-[#10b981] border-[#10b981]/25 hover:bg-[#10b981]/20' : 'bg-emerald-50 text-emerald-700 border-emerald-250 hover:bg-emerald-100'
                }`}
              >
              ...
              <button
                ...
                onClick={triggerFileSelect}
                className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-[10.5px] font-bold transition active:scale-95 cursor-pointer border shadow-sm ${
                  isDark ? 'bg-slate-800 border-slate-700 text-slate-350 hover:text-white hover:bg-slate-750' : 'bg-white border-slate-205 text-slate-650 hover:bg-slate-50 hover:text-slate-800'
                }`}
              >
            ...
            <p className={`text-[9px] max-w-xs leading-relaxed ${isDark ? 'text-[#556b62]' : 'text-slate-450'}`}>
              Hỗ trợ chụp ảnh trực tiếp bằng webcam, camera điện thoại hoặc tải lên hoá đơn giấy tờ, sớ biên nhận
            </p>
          </div>
        )}
          ...
          <div className="p-2 border-t border-amber-500/10 bg-amber-500/5 text-[9px] text-amber-500 font-mono leading-relaxed flex gap-1.5 items-start">
            ...
            <div className="space-y-1">
              ...
              <button
                type="button"
                onClick={triggerFileSelect}
                className="underline font-bold text-amber-600 block hover:text-amber-500"
              >
                👉 Click vào đây để chọn File / Chụp bằng camera điện thoại của bạn
              </button>
            </div>
          </div>
        )}
