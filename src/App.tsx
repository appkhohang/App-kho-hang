...
import { LogOut, User, Bell, Shield, ShieldCheck, Menu, Info, RefreshCw, Layers, CheckCircle2, X, BarChart3, Database, Sun, Moon, HelpCircle, Download, Upload, AlertCircle, Trash2, Settings, FileSpreadsheet, Smartphone, Scissors, Home, TrendingUp, ShoppingCart, FileText, Factory, Calendar, DollarSign, ChevronRight } from 'lucide-react';
...
export default function App() {
  ...
  const getUserRole = (): 'admin' | 'staff' | 'viewer' => {
    ...
    if (email === 'vukuli.123@gmail.com') return 'admin';
    const profile = userProfiles.find(p => p.email.toLowerCase().trim() === email);
    if (!profile) return 'viewer'; // BE SAFE! Lowest level fallback if profile isn't fetched/found yet
    return profile.active === false ? 'viewer' : profile.role;
  };
  ...
  const getUserAllowedTabs = (): string[] => {
    ...
    if (email === 'vukuli.123@gmail.com') return allTabs;
    
    const profile = userProfiles.find(p => p.email.toLowerCase().trim() === email);
    if (!profile) return ['home']; // Safe fallback instead of allTabs!
    
    if (profile.active === false) return [];
    
    if (profile.allowedTabs) {
      return profile.allowedTabs;
    }
    
    // Legacy support for older profile formats without explicitly defined allowedTabs field
  ...
  useEffect(() => {
    ...
    const fetchUserProfile = async () => {
      const email = authState.email?.toLowerCase().trim();
      if (!email || !authState.isAuthenticated) return;
      
      try {
        const docRef = doc(db, 'user_profiles', authState.email);
        let docSnap = await getDoc(docRef);
        
        // Try exact lowercase if not found as is
        if (!docSnap.exists() && authState.email.toLowerCase() !== authState.email) {
          const lowerDocRef = doc(db, 'user_profiles', authState.email.toLowerCase());
          docSnap = await getDoc(lowerDocRef);
        }
        ...
        if (docSnap.exists() && active) {
          ...
          setUserProfiles(prev => {
            ...
            return [profileData, ...filtered];
          });
        }
      } catch (err) {
        console.warn("Failed to fetch user profile directly, relying on synced state", err);
      }
    };
  ...
  React.useEffect(() => {
    if (authState.isAuthenticated && allowedTabs.length > 0) {
      if (activeTab !== 'notifications' && !allowedTabs.includes(activeTab)) {
  ...
  useEffect(() => {
    ...
    const colors = colorMap[primary] || colorMap.blue;
                ...
                <div className="relative">
                  ...
                  <AnimatePresence>
                    {showNotificationsDropdown && (
                      <div className="absolute right-0 mt-2.5 w-80 bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl p-4 overflow-hidden z-50">
                        <div className="flex justify-between items-center border-b border-slate-105 dark:border-slate-800 pb-2.5 mb-2.5 text-xs">
                          <span className="font-bold text-slate-800 dark:text-slate-200">HOẠT ĐỘNG ĐĂNG NHẬP (LỊCH SỬ)</span>
                          <button
                        ...
                        <div className="space-y-2 max-h-[250px] overflow-y-auto pr-1">
                          {authState.loginNotifications.length === 0 ? (
                            ...
                            authState.loginNotifications.map(notif => (
                              <div
                                key={notif.id}
                                className="p-2.5 bg-slate-50 dark:bg-slate-900/60 border border-slate-150 dark:border-slate-800 rounded-xl relative group text-[11px]"
                              >
                                <button
                                  onClick={() => deleteNotification(notif.id)}
                                  className="absolute right-2 top-2 text-slate-450 hover:text-red-505 opacity-0 group-hover:opacity-100 transition"
                                ...
                                <div className="flex gap-1.5 text-[10px] text-slate-400 font-mono">
                                  {notif.device.includes("Hệ thống tự động") ? (
                                    <RefreshCw className="w-3 h-3 mt-0.5 text-emerald-500 animate-spin-slow" />
                                  ) : (
                                    <Shield className="w-3 h-3 mt-0.5 text-indigo-400" />
                                  )}
                                  <span>{notif.time}</span>
                                </div>
                                <p className="font-bold text-slate-800 dark:text-slate-200 mt-1">
                                  {notif.device.includes("Hệ thống tự động") ? "🔄 Đồng bộ ngày giờ & dữ liệu" : "🔐 Đăng nhập hệ thống"}
                                </p>
                                <p className="text-slate-500 mt-0.5">
                                  Trạng thái: <span className="font-semibold text-emerald-600 dark:text-emerald-400">{notif.ip}</span>
                                </p>
                                <p className="text-slate-500">Khu vực: <span className="font-medium text-slate-600 dark:text-slate-350">{notif.location}</span></p>
                                <p className="text-[10px] font-medium text-slate-400 mt-1 font-mono bg-slate-100 dark:bg-zinc-900 p-1 rounded-sm border border-slate-200/50 dark:border-slate-805">{notif.device}</p>
                              </div>
                            ))
                          )}
                        </div>
